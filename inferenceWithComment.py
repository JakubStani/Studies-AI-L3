...
class PhilipsEspressoMachine(KnowledgeEngine):
    ...
    #poniższe reguły komentują stan wybranego elementu /
    #dają ogólną sugestię, co użytkownik powinien zrobić,
    #aby doprowadzić do właściwego stanu elementu

    #gdy action='start' (program został uruchomiony),
    #gdy element='urządzenie' (wybranym elementem jest całe urządzenie),
    #i stan wybranego elementu, to stan='przegrzane' ...
    @Rule(
        AND(
            Fact(action='start'),
            Fact(element='urządzenie'),
    ...
    def coolDevice(self):
        ...
        #... drukowane jest sugerowane ogólne działanie
        print(f'Powinieneś schłodzić urządzenie!')

    #poniższe reguły są analogiczne do powyższej
